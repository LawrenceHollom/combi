        ...
        match parse_infix_like(text) {
            Some((par1, op, par2)) => {
                ...
                if let Some(infix) = num_to_bool_infix {
                    ...
                    match IntOperation::of_string_result(par1).and_then(|op1| {
                        IntOperation::of_string_result(par2).map(|op2| IntInfix(infix, op1, op2))
                    }) {
                        Some(op) => Some(op),
                        None => RationalOperation::of_string_result(par1).and_then(|op1| {
                            RationalOperation::of_string_result(par2)
                                .map(|op2| FloatInfix(infix, op1, op2))
                        }),
                    }
                } else if let Some(infix) = bool_infix {
                    Self::of_string_result(par1).and_then(|op1| {
                        Self::of_string_result(par2)
                            .map(|op2| BoolInfix(infix, Box::new(op1), Box::new(op2)))
                    })
                } else {
                    None
                }
            ...
            None => {
                ...
                match func.trim().to_lowercase().as_str() {
                    "not" | "!" | "¬" => {
                        Self::of_string_result(args[0]).map(|op| Not(Box::new(op)))
                    }
                    ...
                    "has_regular_lossless_edge_dominator" | "has_rled" => {
                        Some(HasRegularLosslessEdgeDominator)
                    }
                    "true" => Some(Const(true)),
                    "false" => Some(Const(false)),
                    "is_k_connected" | "k_connected" | "k_conn" => {
                        Some(IsKConnected(args[0].parse().unwrap()))
                    }
                    "triangle_free" | "tri_free" | "k3_free" => Some(IsTriangleFree),
                    "is_forest_and_matching" | "ifam" => {
                        Some(CanBeEdgePartitionedIntoLinearForestAndMatching)
                    }
                    ...
                    "chromatic_game_strongly_monotone" | "chi_g_sm" => {
                        Some(IsChromaticGameStronglyMonotone)
                    }
                    ...
                    "chi_g_subsets" => {
                        Some(ChromaticGameSubsetMonotonicity(args[0].parse().unwrap()))
                    }
                    ...
                    "alice_greedy_win" => {
                        Some(MakerWinsChromaticGameGreedily(args[0].parse().unwrap()))
                    }
                    ...
                    "bunkbed_sites_conditioning" | "bb_sites_cond" => {
                        Some(BunkbedSiteHasBadConditioning)
                    }
                    ...
                    "incomp_connected" | "is_incomp_connected" => {
                        Some(IsPosetIncomparabilityConnected)
                    }
                    ...
                    "num_extensions_less_than" => {
                        Some(NumLinearExtensionsLessThan(args[0].parse().unwrap()))
                    }
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        let name = match self {
            ...
            GameChromaticWinnerWithDuds(k) => {
                format!(
                    "Whether Maker wins the chromatic game on {} colours (one of which is a dud)",
                    k
                )
            }
            ChromaticGameSubsetMonotonicity(k) => {
                format!(
                    "Whether the chromatic game has subset-monotonicity from k = {} to {}",
                    k,
                    k + 1
                )
            }
            LinearGameChromaticWinner(k) => {
                format!(
                    "Whether Maker wins the linear chromatic game on {} colours",
                    k
                )
            }
            GameGrundyWinner(k) => {
            ...
            MakerWinsConnectedChromaticGame(k) => {
                format!(
                    "Whether Maker wins the connected chromatic game on {} colours",
                    k
                )
            }
            ...
            MakerWinsChromaticGameGreedily(k) => {
                format!(
                    "Whether Maker wins chromatic game with {} colours playing greedily",
                    k
                )
            }
            ...
            CanBobWinGraphGrabbing(max_weight) => {
                format!(
                    "Can Bob with the graph grabbing game with max_weight {:?}",
                    max_weight
                )
            }
            IsOddCoronaFree => {
                "Has no induced corona product of an odd cycle and a point".to_owned()
            }
            ...
            ContradictsBunkbedSiteConjecture => {
                "Does this contradict the bunkbed sites conjecture".to_owned()
            }
            ContradictsReducedBunkbedConjecture => {
                "Does this contradict the reduced bunkbed conjecture".to_owned()
            }
            ApproxContradictsReducedBunkbedConjecture(samples) => {
                format!(
                    "Approx contradicts the reduced bunkbed conjecture with {} samples",
                    samples
                )
            }
            ContradictsReducedConditionedBunkbedConjecture => {
                "Does this contradict the reduced conditioned bunkbed conjecture".to_owned()
            }
            IsBunkbedReducible => "Is bunkbed reducible".to_owned(),
            BunkbedGadget => {
                "Use as a gadget in the search for a bunkbed counterexample".to_owned()
            }
            ...
            KuramotoSynchronises(k) => format!(
                "Does Kuramoto synchronise under a random start ({} attempts)?",
                k
            ),
            KuramotoSimplest(k) => format!(
                "Does Kuramoto synchronise, and find simplest unsyncd ({} attempts)?",
                k
            ),
            ...
            IsSiteKozmaNitzanFalse => {
                "Is the site-percolation Kozma--Nitzan conjecture false".to_owned()
            }
            ...
            IsGenericallyHyperbolicEmbeddable(_) => {
