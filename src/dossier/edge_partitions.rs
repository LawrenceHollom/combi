...
fn minimax_forest_analyse_rec(
    g: &Graph,
    colours: &mut EdgeVec<u8>,
    next_vert: Vertex,
    other_end: &mut EdgeVec<Option<Edge>>,
    path_len: &mut EdgeVec<usize>,
    max_len_here: usize,
    best_len: Option<usize>,
    edges: &[Edge],
    part_two_max_len: Option<usize>,
    long_path_count: usize,
    long_path_cap: Option<usize>,
) -> Option<usize> {
    ...
    if i != j && local_cols[i] == local_cols[j] {
        ...
        value = opn_min(
            value,
            minimax_forest_set_colours_rec(
                g,
                colours,
                next_vert.incr(),
                other_end,
...
fn minimax_forest_set_colours_rec(
    g: &Graph,
    colours: &mut EdgeVec<u8>,
    next_vert: Vertex,
    other_end: &mut EdgeVec<Option<Edge>>,
    ...
    let edges: Vec<Edge> = g.adj_list[next_vert]
        .iter()
        .map(|x| Edge::of_pair(next_vert, *x))
        .collect();
    let local_cols: Vec<u8> = edges.iter().map(|e| colours.get(*e)).collect();
    let mut value = best_len;
...
pub fn count_bipartite_edge_bisections(g: &Graph) -> u32 {
    ...
    'test_edgeset: for blue_edges in g.iter_edge_sets() {
        ...
        if let (Some(max_blue_size), Some(max_red_size)) = (
            blue_sizes.max(&0, usize::cmp),
            red_sizes.max(&0, usize::cmp),
        ) {
