...
fn find_interval_coloring_rec(
    g: &Graph,
    vert_ordering: &Vec<Vertex>,
    colored: &mut VertexVec<bool>,
    colors: &mut EdgeVec<Option<i32>>,
    num_colored: usize,
) -> Option<EdgeVec<Option<i32>>> {
    if num_colored == g.n.to_usize() {
        ...
        for start in (max_col - deg + 1)..(min_col + 1) {
            ...
            if d == 0 {
                let coloring =
                    find_interval_coloring_rec(g, vert_ordering, colored, colors, num_colored + 1);
                if coloring.is_some() {
                    return coloring;
                }
            } else {
                ...
                for _perm_num in 0..factorial(d) {
                    ...
                    let coloring = find_interval_coloring_rec(
                        g,
                        vert_ordering,
                        colored,
                        colors,
...
pub fn print_interval_coloring(g: &Graph) {
    ...
    match find_interval_coloring_rec(
        g,
        &vert_ordering,
        &mut VertexVec::new(g.n, &false),
        &mut colors,
        0,
    ) {
        Some(coloring) => {
