...
fn flat_paths_must_flip_at_post(
    g: &Graph,
    u: Vertex,
    v: Vertex,
    posts: VertexSet,
    ...
    are_connected(g, u, false, v, false, posts, config, indexer)
        && !are_connected(
            ...
            v,
            false,
            posts.remove_vert_immutable(post),
            config,
            indexer,
        )
}

fn cross_paths_must_flip_at_post(
    g: &Graph,
    u: Vertex,
    v: Vertex,
    posts: VertexSet,
    ...
    for v in g.n.iter_verts().skip(1) {
        ...
        for post in g.n.iter_verts() {
            ...
            for config in g.iter_edge_sets() {
                if flat_paths_must_flip_at_post(g, u, v, posts, post, &config, &indexer) {
                    counter -= 1;
                }
                if cross_paths_must_flip_at_post(g, u, v, posts, post, &config, &indexer) {
                    counter += 1;
                }
            }
            if verbose {
...
pub fn contradicts_reduced_conditioned_bunkbed_conjecture(g: &Graph) -> bool {
    ...
    for config in g.iter_edge_sets() {
        let connections =
            equivalence_relation::get_connections(&g_etc, Vertex::ZERO, &config, &indexer);
...
pub fn contradicts_reduced_bunkbed_conjecture(g: &Graph) -> bool {
    ...
    print_vertex_table(vec![
        ("posts", g_etc.posts_for_table()),
        ("flat", num_flat.to_vec_of_strings()),
        ("cross", num_cross.to_vec_of_strings()),
    ]);
...
fn remove_vertex(
    ...
    vert_activity: &mut VertexVec<Option<usize>>,
    num_active_verts: &mut usize,
) {
...
fn compute_equivalence_counts(g_etc: &GraphAndMetadata, edge_type: EdgeType) -> EquivalenceCounts {
    ...
    if PRINT_DEBUG_LEVEL >= 1 {
        ...
        print_vertex_table(vec![
            ("posts", g_etc.posts_for_table()),
            ("targets", g_etc.targets_for_table()),
        ]);
    }
    ...
    for v in g_etc.iter_verts_bfs().skip(1) {
        ...
        if PRINT_DEBUG_LEVEL >= 1 {
            print!(
                "Starting on vertex {}; num_active_verts = {}, num RERs = {}; ",
                v,
                num_active_verts,
                counts.num_distinct_rers()
            );
    ...
    if g_etc.num_targets() >= 3 {
        ...
        for i in 0..g_etc.num_targets() {
            ...
            remove_vertex(
                i,
                &mut counts_copy,
                &mut VertexVec::new(g_etc.get_n(), &None),
                &mut 3,
            );
...
fn simulate_connection_count_ratios(
    h: &Graph,
    num_reps: usize,
    k: usize,
        ...
        if h.constructor.is_random() && rng.gen_bool(0.9) {
            'find_interesting_g_etc: loop {
                ...
                if !g_etc.is_boring() {
                    if PRINT_DEBUG_LEVEL >= 2 {
                        println!(
                            "Found interesting graph; min_sum = {}",
                            g_etc.min_distance_sum()
                        );
                        g_etc.print();
                    }
    ...
    if num_reps > 2 {
        ...
        println!(
            "Average {} boring graphs per interesting one",
            num_boring / num_reps
        );
    }
}
...
pub fn is_contradictory_3_gadget(h: &Graph) -> bool {
    ...
    fn amalgamate_3_edge(
        counts: &mut EquivalenceCounts,
        gadget_counts: &EquivalenceCounts,
        vert_activity: &mut VertexVec<Option<usize>>,
        x: Vertex,
        y: Vertex,
        z: Vertex,
    ...
    remove_vertex(
        vert_activity[v(5)].unwrap(),
        &mut counts,
        &mut vert_activity,
        &mut num_active_verts,
    );
    remove_vertex(
        vert_activity[v(6)].unwrap(),
        &mut counts,
        &mut vert_activity,
        &mut num_active_verts,
    );
    ...
    remove_vertex(
        vert_activity[v(1)].unwrap(),
        &mut counts,
        &mut vert_activity,
        &mut num_active_verts,
    );
    remove_vertex(
        vert_activity[v(2)].unwrap(),
        &mut counts,
        &mut vert_activity,
        &mut num_active_verts,
    );
    ...
    remove_vertex(
        vert_activity[v(7)].unwrap(),
        &mut counts,
        &mut vert_activity,
        &mut num_active_verts,
    );
    remove_vertex(
        vert_activity[v(8)].unwrap(),
        &mut counts,
        &mut vert_activity,
        &mut num_active_verts,
    );
