...
impl ReducedEquivalenceRelation {
    pub fn of_raw_vecs(raw: &[[usize; 2]; 2]) -> Self {
        ...
        let next_label =
            EquivalenceClass::new(raw[0][0].max(raw[0][1]).max(raw[1][0]).max(raw[1][1]) + 1);
        Self {
            k: 2,
            down,
            up,
    ...
    pub fn empty(k: usize) -> Self {
        ...
        Self {
            ...
            down,
            up,
            next_label: EquivalenceClass::new(2 * k),
        }
    }
    ...
    pub fn reduce_no_symmetrise_slow(&self, is_flat: bool) -> Self {
        ...
        let mut next_label = EquivalenceClass::ZERO;
        fn check_label(
            labels: &mut Vec<Option<EquivalenceClass>>,
            next_label: &mut EquivalenceClass,
            comp: EquivalenceClass,
        ) {
    ...
    pub fn get_all_permutations(&self) -> Vec<Self> {
        ...
        for sigma in sigmas.iter() {
            ...
            permutations.push(
                rer.reduce_no_symmetrise_slow(true)
                    .min(rer.reduce_no_symmetrise_slow(false)),
            )
        }
        permutations
    }
...
mod tests {
    ...
    fn test_reduce_no_symmetrise() {
        ...
        assert_eq!(
            rer.reduce_no_symmetrise_slow(true).reduce_and_symmetrise(),
            rers("000102")
        );
    }
