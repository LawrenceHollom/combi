...
impl GraphAndMetadata {
    ...
    pub fn new_forced(
        h: &Graph,
        targets: VertexSet,
        forced_posts: VertexSet,
        forced_non_posts: VertexSet,
    ) -> Self {
    ...
    pub fn is_boring(&self) -> bool {
        ...
        if self
            .g
            .num_filtered_components(Some(&self.posts.not().to_vec()))
            >= 2
        {
            return true;
        }
        if self
            .g
            .num_filtered_components(Some(&self.targets.not().to_vec()))
            >= 2
        {
            return true;
        }
    ...
    pub fn print_table(&self) {
        print_vertex_table(vec![
            ("posts", self.posts_for_table()),
            ("targets", self.targets_for_table()),
        ]);
    }
