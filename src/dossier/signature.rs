...
fn print_general_signature(mut dossier: Dossier, int_operations: Vec<IntOperation>, bool_operations: Vec<BoolOperation>) {
    let mut properties: Vec<(String, String)> = vec![
        ("SIGNATURE".to_owned(), "VALUE".to_owned()), ("~~~~".to_owned(), "~~~~".to_owned())];
    let mut ann = crate::dossier::AnnotationsBox::new();
    
    for op in int_operations.iter() {
        properties.push((format!("{}", op), format!("{}", dossier.operate_int(&mut ann, op))));
    }
    for op in bool_operations.iter() {
        properties.push((format!("{}", op), format!("{}", dossier.operate_bool(&mut ann, op))));
    }
